...
foreach ($ventas as $venta) {
    if (sizeof($venta['unidades']) > 0):

        foreach ($venta['unidades'] as $detalle_unidad) {

            if($detalle_unidad->cantidad!='' && $detalle_unidad->cantidad>0){
...
$totaldescuentostablaveta = $ventas[0]['descuento_valor'] +
    $ventas[0]['descuento_porcentaje'];
...
$pdf->Ln(5);
$pdf->Cell(180, 4, "AUTORIZACION NUMERACION SEGUN RESOLUCION No " . $resolucion_numero . " del " . $resolucion_fech_aprobacion, '', '', 'C');
$pdf->Ln(5);
$pdf->Cell(180, 4, "DEL " . $resolucion_prefijo. "-" . $resolucion_numero_inicial . " AL " . $resolucion_prefijo . "-" . $resolucion_numero_final, '', '', 'C');
$pdf->Ln(5);
...
$pdf->Output($_SERVER['DOCUMENT_ROOT'].RUTA_COTIZACIONES .$file_name, 'F');