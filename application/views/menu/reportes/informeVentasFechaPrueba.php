                ...
                <div class="col-md-3">
                    <select id="tipos_venta_select" class="form-control">
                        ...
                        if(count($tipos_venta)>0){
                            foreach($tipos_venta as $row){ ?>
                                <option value="<?= $row['tipo_venta_id'] ?>"><?= $row['tipo_venta_nombre'] ?></option>
                            <?php }
                        }
                        ?>
                    </select>
                    ...
                    <div class="table-responsive" id="">
                        <table class="table table-striped dataTable table-bordered" id="tabla">
                            ...
                            <tr>
                                ...
                                <th colspan="2">Base Excluida</th>
                                <th colspan="2">Base Gravada</th>
                                <th rowspan="2">Descuento Total</th>
                                <?php foreach ($impuestos as $impuesto) {
                                    ?>

                                    <th rowspan="2"><?= $impuesto['nombre_impuesto'] ?></th>

                                    <?php if ($impuesto['tipo_calculo'] != 'FIJO') { ?>
                                        <th rowspan="2">Gravado <?= $impuesto['nombre_impuesto'] ?></th>
                                    <?php } ?>
                                    <?php
                                } ?>
...
<script type="text/javascript">
    ...
    function get_ventas() {
        ...
        TablesDatatablesLazzy.init('<?php echo $ruta ?>api/Venta/ventas_por_fecha_prueba', 0, 'tabla', {
            ...
            [
                {
                    ...
                    action: function (e, dt, node, config) {
                        ...
                        window.location.href = baseurl + 'reportesExcel/excelInformeVentasPorFecha?fecha_desde=' + fercha_desde + '&fecha_hasta=' + fercha_hasta+'&tipos_venta_select='+$("#tipos_venta_select").val()+'&tipos_venta_text'+$("#tipos_venta_select option:selected").text();
...

                {
                    ...
                    action: function (e, dt, node, config) {
                        ...
                        window.location.href = baseurl + 'reportesPdf/pdfInformeVentasPorFecha?fecha_desde=' + fercha_desde + '&fecha_hasta=' + fercha_hasta+'&tipos_venta_select='+$("#tipos_venta_select").val()+'&tipos_venta_text'+$("#tipos_venta_select option:selected").text();
