...
<input type="hidden" 
	value="<?= isset($cliente)? $cliente['estados_id']:'' ?>" 
	id="estado_id_hidden" 
/>
		...
		<div class="modal-body">
			<form name="formagregar" action="<?php echo base_url(); ?>cliente/guardar" method="post" id="formagregar">
							...
							<select name="vendedor" id="vendedor" required="true" class="form-control"
								...
								<?php foreach ($vendedores as $vendedor) : ?>
									<option 
										...
										<?php if (isset($cliente['vendedor_a']) and $cliente['vendedor_a'] == $vendedor['nUsuCodigo']) echo 'selected' ?>><?= $vendedor['nombre'] ?>
									</option>
								<?php endforeach ?>
							</select>
						</div>
				...
				<div class="row">
					...
					<div class="col-md-4">
						<input type="text" name="direccion" id="location" class="form-control" title="Dirección" autocomplete="on" value="<?php if (isset($cliente['direccion'])) echo $cliente['direccion']; ?>">
					</div>
				</div>
				<hr>
				<div class="row">
