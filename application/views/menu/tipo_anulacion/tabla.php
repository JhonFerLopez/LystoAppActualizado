...
<div class="row bg-title">
	<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
		<h4 class="page-title">Tipos de anulación</h4>
	</div>
				...
				<table class="table table-striped dataTable table-bordered" id="example">
					...
					<tbody>
						<?php if (count($tipos) > 0) {
							...
							?>
								<tr>
									<td class="center"><?= $tipo['tipo_anulacion_id'] ?></td>
									<td><?= $tipo['tipo_anulacion_nombre'] ?></td>
									<td class="center">
										<div class="btn-group">
											<?php
											echo '<a class="btn btn-default" data-toggle="tooltip"
                                            title="Editar" data-original-title="fa fa-comment-o"
                                            href="#" onclick="editarempresa(' . $tipo['tipo_anulacion_id'] . ');">'; ?>
											<i class="fa fa-edit"></i>
											</a>
											<?php echo '<a class="btn btn-default" data-toggle="tooltip"
                                     title="Eliminar" data-original-title="fa fa-comment-o" onclick="borrarempresa(' . $tipo['tipo_anulacion_id'] . ',\'' . $tipo['tipo_anulacion_nombre'] . '\');">'; ?>
...
<script type="text/javascript">
	...
	let barloadermodal = $("#barloadermodal");
	...
	function editarempresa(id) {
		...
		$.ajax({
			url: '<?= $ruta ?>tipo_anulacion/form/' + id,
			type: 'post',
	...
	var objeto = {
		ajax: function() {
			return $.ajax({
				url: '<?= base_url() ?>tipo_anulacion'
			})
		},
		guardar: function() {
			$("#guardar").addClass('disabled');

			if ($("#tipo_anulacion_nombre").val() == '') {
				var growlType = 'warning';
...
<div class="modal fade" id="borrar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<form name="formeliminar" id="formeliminar" method="post" action="<?= $ruta ?>tipo_anulacion/eliminar">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title">Eliminar tipo de anulación</h4>
				</div>
				<div class="modal-body">
					<p>Est&aacute; seguro que desea eliminar?</p>
					<input type="hidden" name="tipo_anulacion_id" id="id_borrar">
					<input type="hidden" name="tipo_anulacion_nombre" id="nom_borrar">
				</div>
				<div class="modal-footer">
