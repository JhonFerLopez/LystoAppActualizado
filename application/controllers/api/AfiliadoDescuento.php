...
class afiliadoDescuento extends REST_Controller
{
    protected $uid = null;
    ...
    {
        $data = array();
        $data['empresas_afiliadas'] = $this->afiliado_model->get_all();
    ...
    function show_by_empresa_get()
    {
        $id = $this->get('empresa_id');
        if (empty($id)) {
            $this->response(array(), 200);
        }

        $data = array();
        $data['paises'] = $this->afiliado_descuentos->get_by('afiliado_id', $id);
