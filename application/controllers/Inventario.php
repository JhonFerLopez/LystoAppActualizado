...
class inventario extends MY_Controller
{
        ...
        foreach ($productos as $pd):
            ...
            if (count($todasunidades) > 0) {
                foreach ($todasunidades as $todas) {
                    $cantidad = 0;
                    //inventario del producto
                    $existencia = $this->inventario_model->get_inventario_by_producto_and_unidad($pd['producto_id'], $local,
                        $todas['id_unidad'], $selectinventario);
    ...
    function informeDiferencia($id = FALSE)
    {
        ...
        foreach ($detalles as $detalle) {
            ...
            $detalle->costo_unitario = $this->venta_model->calcularCostos($producto, $cantidad,
    ...
    private function transFormUnidadesRegistro($productos, $post)
    {
        ...
        foreach ($productos as $producto) {
            ...
            $ubicacion[] = $post['ubicacion_producto_' . $producto];
        }
    ...
    function guardar()
    {
        ...
        if (!$this->input->post('tipodocumento')) {
            ...
            $imprimir=false;
            $json = $this->ajusteinventario_model->saveRegistroFisico(REG_FISICO_SID,$id_local,
    ...
    function buscarExistenciayPrecios()
    {
        ...
        if ($is_paquete == '1') {
            ...
            foreach ($productos as $prod) {
                ...
                foreach ($prod['unidades'] as $unidad) {
                    $unidad = $unidad;
                    $unidad['stock'] = $this->inventario_model->get_inventario_by_producto_and_unidad($prod['prod_id'],
                        $local, $unidad['unidad_id'], $selectinventario);
        ...
        if ($this->input->is_ajax_request()) {
            $data['costo_unitario'] = $costo_unitario['costo_unitario'];
    ...
    function parhceStockNegativo()
    {
        ...
        foreach ($productos as $producto) {
            ...
            foreach ($inventarios as $inv) {
                $this->inventario_model->updateOrInsertInventario($inv['id_producto'], $inv['id_unidad'], $inv['id_local'], $inv['id_inventario'],
                    0);
            }
