...
class ReportesPdf extends MY_Controller
{
...
// redireccionamos la salida al navegador del cliente (Excel2007)
    ...
    function pdfInformeVentasPorFecha()
    {
        ...
        $html = '';
        $html .= '<table><tr><td style="text-align: center">SID-SISTEMA INTEGRAL DE DROGUERIAS</td></tr>';
        $html .= '<tr><td style="text-align: center">' . $this->session->userdata('EMPRESA_NOMBRE') . '</td></tr>';
        $html .= '<tr><td style="text-align: center">' . 'NIT:' . $this->session->userdata('NIT') . '</td></tr>';
        $html .= '<tr><td style="text-align: center">DESDE: ' . $fecha_desde . ' HASTA: ' . $fecha_hasta . ' FECHA/HORA: ' . $fecha . ' USUARIO: ' . $usuario . '</td></tr>';
        $html .= '</table><br>';
        $pdf->writeHTMLCell($w = 0, $h = 0, $x = '', $y = '', $html, $border = 0, $ln = 1, $fill = 0, $reseth = true, $align = '', $autopadding = true);

        $impuestos = $this->impuestos_model->get_impuestos();

        $colspan = 9 + sizeof($impuestos);

        $html = '<br><br><table>';
        $html .= '<tr><td rowspan="2">Fecha</td>';

        if ($tipos_venta_text != "TODOS") {
            $html .= '<td rowspan="2">Tipo de Venta</td>';
        }

        $html .= '<th colspan="2">Base Excluida</th><th colspan="2" >Base Gravada</th>
<th rowspan="2">Descuento Total</th>';
        foreach ($impuestos as $impuesto) {

            $html .= '<th rowspan="2">' . $impuesto['nombre_impuesto'] . '</th>';
            $html .= '<th rowspan="2">Gravado' . $impuesto['nombre_impuesto'] . '</th>';
        }

        $html .= '<th rowspan="2">Valor Iva </th ><th rowspan="2" > Anulaciones </th > <th rowspan="2" > Devoluciones </th > <th rowspan="2" > Venta Total </th > ';
        $html .= '</tr > ';
        ...
        $nombre_archivo = utf8_decode("InformeVentasPorecha.pdf");
        $pdf->Output($nombre_archivo, 'D');
    }

    function pdfInformeCompra()
    {
        $fecha_desde = $this->input->get('fecha_desde');
        $fecha_hasta = $this->input->get('fecha_hasta');
        $tipos_venta_select = $this->input->get('tipos_venta_select');
        ...
        foreach ($impuestos as $impuesto) {
            ...
            $html .= '<th>' . $impuesto['nombre_impuesto'] . '</th>';
            $html .= '<th >Gravado' . $impuesto['nombre_impuesto'] . '</th>';
        }

        $html .= '<th >Total Iva </th ><th rowspan="2" > Total gravado </th > <th > Total excluido </th > <th rowspan="2" > Gravado + Excluido </th > <th rowspan="2" > Total </th > ';
        $html .= '</tr > ';
        ...
        $nombre_archivo = utf8_decode("InformeCompras.pdf");
        $pdf->Output($nombre_archivo, 'D');
    }
