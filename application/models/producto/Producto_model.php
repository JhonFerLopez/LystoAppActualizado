...
class producto_model extends CI_Model
{
    ...
    {
        $this->db->distinct();
        $this->db->select('unidades_has_precio.precio,producto.producto_nombre as nombre, ' . $this->tabla . '.*, unidades.nombre_unidad, inventario.id_inventario, inventario.id_local, inventario.cantidad, inventario.fraccion ,
		grupos.nombre_grupo, proveedor.proveedor_nombre, impuestos.nombre_impuesto,grupos.id_grupo');
    ...
    function insertar($producto, $medidas, $unidades, $codigos_barra, $stock_minimo, $stock_maximo, $componentes, $productos_paquete)
    {
                    ...
                    if (isset($medidas[$countunidad])) {
                        ...
                        foreach ($condiciones_de_pago as $condicion_pago) {
                            ...
                            if (isset($precios_id[$countprecio])) {
                                ...
                                $precio_minimo[$condicion_pago['id_condiciones']] != false && $precio_minimo[$condicion_pago['id_condiciones']] != ""
                                    ? $unidad_has_precio['precio_minimo'] = $precio_minimo[$condicion_pago['id_condiciones']] : "";

                                $precio_maximo[$condicion_pago['id_condiciones']] != false && $precio_maximo[$condicion_pago['id_condiciones']] != ""
                                    ? $unidad_has_precio['precio_maximo'] = $precio_maximo[$condicion_pago['id_condiciones']] : "";
        ...
        try {
            ...
            foreach (json_decode($post['lst_producto']) as $row) {
                ...
                if ($row != null && $row->producto_id) {
                    ...
                    $producto_before = ProductoElo::where('producto_id', $row->producto_id)->with(ProductoElo::allTablesRelations())->first();
                                ...
                                for ($m = 0; $m < count($todasUnidades); $m++) {
                                    ...
                                    if (isset($row->precio_abierto->{$condiciones[$i]['id_condiciones']}->{$todasUnidades[$m]['id_unidad']})) {
                                        ...
                                        if (
                                            ...
                                            $preciominimo = "";
                                        } elseif (isset($row->precio_abierto->{$condiciones[$i]['id_condiciones']}->{$todasUnidades[$m]['id_unidad']}->precio_minimo) and
                                            $row->precio_abierto->{$condiciones[$i]['id_condiciones']}->{$todasUnidades[$m]['id_unidad']}->precio_minimo != ""
                                        ) {
                                            $preciominimo = $row->precio_abierto->{$condiciones[$i]['id_condiciones']}->{$todasUnidades[$m]['id_unidad']}->precio_minimo;
                                        }

                                        if (isset($row->precio_abierto->{$condiciones[$i]['id_condiciones']}->{$todasUnidades[$m]['id_unidad']}->precio_maximo)
                                            and $row->precio_abierto->{$condiciones[$i]['id_condiciones']}->{$todasUnidades[$m]['id_unidad']}->precio_maximo == ""
                                        ) {
                                            $preciomaximo = "";
                                        } elseif (isset($row->precio_abierto->{$condiciones[$i]['id_condiciones']}->{$todasUnidades[$m]['id_unidad']}->precio_maximo) and
    ...
    {
        $this->db->select('producto.*,impuestos.porcentaje_impuesto, impuestos.tipo_calculo as tipo_impuesto, otroimp.porcentaje_impuesto as porcentaje_otro_impuesto, 
         otroimp.tipo_calculo as tipo_otro_impuesto, impuestos.fe_impuesto,otroimp.fe_impuesto as  fe_otro_impuesto ');
    ...
    public
    function count_all_catalogo($filter = null)
    {
    ...
    public
    function total_result($select = false, $from = false, $join = false, $campos_join = false, $tipo_join, $where = false, $nombre_in, $where_in,
                          $nombre_or, $where_or,
                          $group = false,
                          $order = false, $retorno = false, $limit = false, $start = 0, $order_dir = false, $like = false, $where_custom)
    {
