...
class control_ambiental_model extends CI_Model
{
	...
	{
		$nombreExists = $this->get_only_control(array('periodo' => $control['periodo']));
		$validar_mes = is_array($nombreExists) ? sizeof($nombreExists) : 0;
	...
	function update_control($control)
	{
		...
		$validar_mes = is_array($buscar) ? sizeof($buscar) : 0;
	...
	function guardardetalle($control, $post)
	{
		...
		$número = cal_days_in_month(
			CAL_GREGORIAN,
			date("m", strtotime($control['periodo'])),
			date("Y", strtotime($control['periodo']))
		);
		$this->db->trans_start();
		for ($i = 1; $i <= $número; $i++) {
			$datos = array();
