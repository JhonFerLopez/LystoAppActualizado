...
class usuarios_grupos_model extends CI_Model
{
	...
	function get_all()
	{
		...
		$query = $this->db->where('nombre_grupos_usuarios <>', 'PROSODE_ADMIN');
		$query = $this->db->get('grupos_usuarios');
		return $query->result_array();
	}

	function getUsuariosByRolNomb($rol)
	{
		$query = $this->db->where('status_grupos_usuarios', 1);
		$query = $this->db->where('nombre_grupos_usuarios <>', 'PROSODE_ADMIN');
		$query = $this->db->where('nombre_grupos_usuarios', $rol);
		$query = $this->db->join('usuario', 'usuario.grupo = grupos_usuarios.id_grupos_usuarios');
		$query = $this->db->get('grupos_usuarios');
		return $query->result();
	}
	...
	public function edit_role($role_id, $data)
	{
		...
		$validar_nombre = is_array($produc_exite) ? sizeof($produc_exite) : 0;
	...
	{
		$nombre = $data['nombre_grupos_usuarios'];
		$produc_exite = $this->get_by('nombre_grupos_usuarios', $nombre);
		$validar_nombre = is_array($produc_exite) ? sizeof($produc_exite) : 0;
