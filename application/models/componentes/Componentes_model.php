...
class componentes_model extends CI_Model {
    ...
    function set($data)
    {
        ...
        $validar_nombre = sizeof($this->get_by(array($this->nombre=> $data['componente_nombre'],$this->deleted=>NULL)));
    ...
    function update($grupo, $quehago = NULL)
    {
        ...
        $produc_exite=$this->get_by(array($this->nombre=>$grupo['componente_nombre'], $this->deleted=>null) );
        $validar_nombre = sizeof($produc_exite);
        if ($validar_nombre < 1 or( $validar_nombre>0 and ($produc_exite [$this->id]==$grupo ['componente_id']))) {
